...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  test("baabu") {
    val requester = TestProbe()
    val requestRef = requester.ref
    val topNode = system.actorOf(Props[BinaryTreeSet])

    topNode ! Insert(requestRef, id=101, -1)
    topNode ! Insert(requestRef, id=102, 1)
    topNode ! Contains(requestRef, id = 103, 1)
    topNode ! Remove(requestRef, id=104, 1)
    topNode ! GC
    topNode ! Contains(requestRef, id = 105, -1)
    topNode ! Contains(requestRef, id = 106, 1)
  }
